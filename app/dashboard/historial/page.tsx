...
export default async function HistorialPage() {
  ...
  const total = predictions.reduce((s: number, p: any) => s + (p.points ?? 0), 0);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          {predictions.length} predicciones · <strong>{total} puntos</strong>
        </p>
      </div>
      ...
      <div className="space-y-2">
          ...
          <div key={p.id} className="bg-white rounded-xl border px-4 py-3 flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-gray-400 mt-0.5">
                Tu predicción: <strong>{p.home_score_pred} - {p.away_score_pred}</strong>
        ...
        {predictions.length === 0 && (
          <p className="text-gray-400 text-sm text-center py-8">Todavía no hiciste ninguna predicción.</p>
        )}
      </div>
    </div>
  );
