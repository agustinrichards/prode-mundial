...
export default async function AdminTarjetasPage() {
  ...
  const myPredictions = await query(`
    SELECT m.id AS match_id, m.home_team, m.away_team, m.match_date,
      ...
      NULL AS home_score_pred, NULL AS away_score_pred, NULL AS points, FALSE AS locked,
      FALSE AS co2, FALSE AS rio, NULL AS rio_home, NULL AS rio_away
    FROM matches m
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Todas las predicciones — vista admin</p>
      </div>
      <MisApuestasClient
