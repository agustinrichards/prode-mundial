...
export function MatchesClient({ matches, userId, co2Usage, rioUsage }: Props) {
  ...
  const now = new Date();
  const isClosed = (m: Match) => isBefore(new Date(m.predictions_close_at), now);
  ...
  const confirmDatePredictions = async (dateLabel: string, dateMatches: Match[]) => {
    // Validate all open matches have predictions
    ...
    if (missingPreds.length > 0) {
      toast({
        ...
        description: `Completá todos los partidos antes de confirmar (faltan ${missingPreds.length})`,
        variant: "destructive"
      });
      return;
    }
          ...
          <div key={dateLabel} className="space-y-2">
            ...
            {!allHavePreds && !dateConfirmed && !allClosed && openMatches.length > 0 && (
              <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-xs text-amber-700">
                ...
                Completá todos los partidos para poder confirmar ({openMatches.filter(m => {
                  const inp = getInput(m);
                  return m.home_score_pred === null && (inp.home === "" || inp.away === "");
                }).length} sin predicción)
              </div>
            )}
                ...
                <div key={match.id} className={`bg-white rounded-xl border p-4 transition-all ${
                  ...
                  {!closed && match.home_score === null && (
                    <div className="flex justify-center mt-1.5 h-4">
                      {isSavingMain ? (
                        ...
                        <span className="text-xs text-gray-300">Sin predicción</span>
                      )}
                    </div>
                  )}
                  ...
                  {match.rio_used && !closed && !locked && (
                    <div className="mt-3 bg-blue-50 rounded-lg p-3">
                      <p className="text-xs text-blue-700 font-semibold mb-2">RIO — segunda predicción</p>
                      <div className="flex items-center gap-2">
